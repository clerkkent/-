(function($, window) {
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			footer: '<span class="mui-preview-indicator"></span>'
		}, options || {});
		this.init();
		this.initEvent();
	};
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			...
			self.indicator && (self.indicator.innerHTML = '<span class=\'current\'>' + (slideNumber + 1) + '</span>/' + self.currentGroup.length);
	...
	proto.refresh = function(index, groupArray) {
		...
		this.indicator && (this.indicator.innerHTML = '<span class=\'current\'>' + (currentIndex + 1) + '</span>/' + this.currentGroup.length);
