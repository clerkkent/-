...
import { delBbsThreadCollect } from '@/util/api'
import CollectionListItem from '@/containers/CollectionListItem'
...
class collectionList extends Component {
  ...
  componentWillMount () {
    const { collectListList } = this.props
    if (collectListList && collectListList.length === 0) {
      this.props.dispatch(actions.fetchBbsUserCollectList({
        pageNum: 10,
        page: '1',
        cb: () => {
          this.initMui()
  ...
  initPullRefresh (bol) {
    ...
    this.setState({
      ...
      delTxt: '全部',
      delAllStatus: 0,
      delBtn: collectListList.length > 0 ? 1 : 0,
      topDel: '删除'
    })
    var that = this
    mui('#refreshContainer_0').pullRefresh({
      down: {
        ...
        callback: function () {
          that.props.dispatch(actions.fetchBbsUserCollectList({
            pageNum: 10,
            page: '1',
            refresh: true,
            cb: (res) => {
      ...
      up: {
        ...
        callback: function () {
          const { collectListPage } = that.props
          let page = parseInt(collectListPage) + 1
          that.props.dispatch(actions.fetchBbsUserCollectList({
            pageNum: 10,
            page: page,
            cb: (res) => {
    ...
    this.setState({
      ...
      if (this.state.checkEd.length > 0) {
        this.Alert.show({
          content: '确定要清空所选消息吗？',
          confirm: () => {
            delBbsThreadCollect(this.state.checkEd).then(res => {
  ...
  render () {
    const { initCollectState, collectListList } = this.props
    return (
      <div className={styles['list-main']}>
        <div className="home-slider mui-slider mui-fullscreen">
          <Navbar
              titleContent='我的收藏'
              rightContent={ (collectListList.length > 0 || this.state.delBtn === 1) && <NavbarDel delInfo = {this.delInfo} topDel = {this.state.topDel} />}
              titleClass="flex-3"
            />
            ...
            <div id='scrollWrapItem0' className={styles['list-slider-box']} className={'mui-slider-item list-bottom mui-control-content  mui-active' }>
              <div id='refreshContainer_0' className={'mui-scroll-wrapper' + (this.state.delStatus === 'del' ? ' w120' : '')}>
                <div id='box-0' className={styles['box-max']}>
                  { initCollectState && collectListList.length > 0
                      ? collectListList.map((item, index) => {
                        return (
                          <CollectionListItem item={item} key={index} delStatus={ this.state.delStatus } delAllStatus={ this.state.delAllStatus } delCount={ this.delCount }/>
                        )
                      })
                    : <DefaultPanel className={styless['no-collection']} title='暂时还没有收藏喔' ></DefaultPanel>
                  }
                </div>
              </div>
            </div>
...
export default withRouter(connect(({list}) => list)(collectionList))
