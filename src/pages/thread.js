...
import { delBbsUserThread } from '@/util/api'
...
class thread extends Component {
  ...
  componentWillMount () {
    ...
    if (threadList && threadList.length === 0) {
      this.props.dispatch(actions.fetchBbsUserThread({
        pageNum: 10,
        page: '1',
        cb: () => {
          this.initMui()
  ...
  initPullRefresh (bol) {
    ...
    this.setState({
      ...
      delTxt: '全部',
      delAllStatus: 0,
      delBtn: threadList.length > 0 ? 1 : 0,
      topDel: '删除'
    })
    var that = this
    mui('#refreshContainer_0').pullRefresh({
      down: {
        ...
        callback: function () {
          that.props.dispatch(actions.fetchBbsUserThread({
            pageNum: 10,
            page: '1',
            refresh: true,
            cb: (res) => {
      ...
      up: {
        ...
        callback: function () {
          const { threadPage } = that.props
          let page = parseInt(threadPage) + 1
          that.props.dispatch(actions.fetchBbsUserThread({
            pageNum: 10,
            page: page,
            cb: (res) => {
    ...
    this.setState({
      ...
      if (this.state.checkEd.length > 0) {
        this.Alert.show({
          content: '确定要清空所选消息吗？',
          confirm: () => {
            delBbsUserThread(this.state.checkEd).then(res => {
  ...
  render () {
    const { initThreadState, threadList, threadPage } = this.props
    return (
      <div className={styles['list-main']}>
        <div className="home-slider mui-slider mui-fullscreen">
          <Navbar
              titleContent='我的帖子'
              rightContent={ (threadList.length > 0 || this.state.delBtn === 1) && <NavbarDel delInfo = {this.delInfo} topDel = {this.state.topDel} />}
              titleClass="flex-3"
            />
            ...
            <div id='scrollWrapItem0' className={styles['list-slider-box']} className={'mui-slider-item list-bottom mui-control-content  mui-active' }>
              <div id='refreshContainer_0' className={'mui-scroll-wrapper' + (this.state.delStatus === 'del' ? ' w120' : '')}>
                <div id='box-0' className={styles['box-max']}>
                  { initThreadState && threadList.length > 0
                      ? threadList.map((item, index) => {
                        return (
                          <ThreadItem item={item} key={index} delStatus={ this.state.delStatus } delAllStatus={ this.state.delAllStatus } delCount={ this.delCount }/>
                        )
                      })
                    : <DefaultPanel className={styless['no-thread']} title='暂时还没有发帖喔' { ...this.props }><div>去发帖<icon></icon></div></DefaultPanel>
                  }
                </div>
              </div>
            </div>
...
export default withRouter(connect(({list}) => list)(thread))
